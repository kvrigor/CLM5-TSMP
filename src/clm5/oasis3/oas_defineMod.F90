...
contains
  ...
  subroutine oas_definitions_init(bounds)
    ...
    CALL oasis_def_var(oas_id_rh, "CLMRAH1" , grid_id, var_nodims, OASIS_In, OASIS_Real, ierror)  !20
    IF (ierror /= 0) CALL oasis_abort(oas_comp_id, oas_comp_name, 'Failure in oasis_def_var for CLMGSPRE.')
    CALL oasis_def_var(oas_id_rm, "CLMRAM1" , grid_id, var_nodims, OASIS_In, OASIS_Real, ierror)  !21
    IF (ierror /= 0) CALL oasis_abort(oas_comp_id, oas_comp_name, 'Failure in oasis_def_var for CLMGSPRE.')
